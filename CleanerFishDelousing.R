...
t5tots$time <- dplyr::recode(t5tots$time, '1' = 0, '2' = 24, '3' = 48, '4' = 72)
t5tots$treatment <- dplyr::recode(t5tots$treatment, 'con' = 'control', 'LL' = 'Large lumpfish', 'SL' = 'Small lumpfish', 'W' = 'Wrasse')


mutate(t5data, total = cumsum(6:31))

t5data$total <- rowSums(t5data[,6:31])

# summarise data by time, group and replicate tank
t5summ <- t5tots %>%
  group_by(time, tank, treatment, replicate) %>%
  summarise(m_m = mean(total_m), sd_m = sd(total_m), m_f = mean(total_f), sd_f = sd(total_f), tot_m = mean(total), tot_sd = sd(total))

# summarise by time and group with rep as error and draw plot of total lice
t5summ %>%
  group_by(time, treatment) %>%
  summarise(mean_m = mean(m_m), sd_m = sd(m_m), mean_f = mean(m_f), sd_f = sd(m_f), total_mean = mean(tot_m), total_sd = sd(tot_m)) %>%
...
t5summ %>%
  ...
  scale_y_continuous(limits = c(0, 10), name = 'mean lice per fish') +
  scale_x_continuous(breaks = c(0, 24, 48, 72, 96), labels = c('0', '24', '48', '72', '96'), name = 'Time (h)') +
  ggtitle('Male lice') +
  theme_classic() +
  theme(legend.title = element_blank())
