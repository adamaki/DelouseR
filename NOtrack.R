...
#wrasse videos
workingdir <- '/Users/adambrooker/OneDrive - University of Stirling/Tank delousing trials/Wrasse NO tests/C12-S' # change to location of data 
...
start <- 203 # start frame number
end <- 302 # end frame number
rotangle <- 3 # image rotation angle to translate image to cartesian grid
xrange <- c(165,503) # x-axis crop dimensions
...
display(test.img)
test.img[rmask[1]:rmask[2], rmask[3]:rmask[4],] <- 0 # mask out light reflection
display(test.img)
...
system.time({
  ...
  for(n in 1:dim(thresh_stack)[[3]]){
    ...
    overlay <- drawCircle(overlay, centre[[1]], centre[[2]], radius = outrad, col = 'yellow', fill = F) # paint tank outline in yellow
...
system.time({
  ...
  if(noin == T){
    # get min fish distance to novel object
